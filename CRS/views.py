        ...
        for data in facultySchedule:
            ...
            if dbDay == AssignedDay:
                if AssignedTimeIn == dbTimeIn:
                    ...
                    validSched = False
                    errorSched = "Schedule Already Exist"
                    break
                if AssignedTimeIn >= dbTimeOut: #Meaning Sched is Later than in the database
                    ...
                    if AssignedTimeOut <= dbTimeIn: #Sched is Earlier than test Schedule
                        ...
                        validSched = False
                        errorSched = "Schedule Overlap at " + dbSubject
                        break
            else:
                validSched = True
        else:
            ...
            if (request.method=='POST'):
                ...
                if total_teaching_minutes >= 90 and total_teaching_minutes <= 360:
                    ...
                    if total_teaching_minutes < 90:
                        messages.error (request,'Time is below minimum hours! (time )')
                    else:
...
def fStudents_viewStudentGrade (request,stud_id):
    if request.user.is_authenticated and request.user.is_faculty:
        year1 = curriculumInfo.objects.filter(schoolYear='1')
        year2 = curriculumInfo.objects.filter(schoolYear='2')
        year3 = curriculumInfo.objects.filter(schoolYear='3')
        year4 = curriculumInfo.objects.filter(schoolYear='4')
        sem1 = curriculumInfo.objects.filter(schoolSem='1')
        sem2 = curriculumInfo.objects.filter(schoolSem='2')
        semSummer = curriculumInfo.objects.filter(schoolSem='Summer')
        checklist = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year1).filter(curriculumCode__in=sem1)
        checklist2 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year1).filter(curriculumCode__in=sem2)
        checklist3 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year2).filter(curriculumCode__in=sem1)
        checklist4 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year2).filter(curriculumCode__in=sem2)
        checklist5 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year3).filter(curriculumCode__in=sem1)
        checklist6 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year3).filter(curriculumCode__in=sem2)
        checklist7 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year4).filter(curriculumCode__in=sem1)
        checklist8 = currchecklist.objects.filter(owner_id=stud_id).filter(curriculumCode__in=year4).filter(curriculumCode__in=sem2)
        try:
            grade_file = crsGrade.objects.get(studentID=stud_id)
        except crsGrade.DoesNotExist:
            grade_file = None
        context = {'checklist': checklist,'checklist2': checklist2,'checklist3': checklist3,'checklist4': checklist4,'checklist5': checklist5,'checklist6': checklist6, 'checklist7': checklist7,'checklist8': checklist8, 'stud_id': stud_id, 'grade_file':grade_file}
...
def sChecklist(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        try:
            ...
            sem2 = curriculumInfo.objects.filter(schoolSem='2')
            semSummer = curriculumInfo.objects.filter(schoolSem='Summer')
            checklist = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year1).filter(curriculumCode__in=sem1)
            checklist2 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year1).filter(curriculumCode__in=sem2)
            checklist3 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year2).filter(curriculumCode__in=sem1)
            checklist4 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year2).filter(curriculumCode__in=sem2)
            checklist5 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year3).filter(curriculumCode__in=sem1)
            checklist6 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year3).filter(curriculumCode__in=sem2)
            checklist7 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year4).filter(curriculumCode__in=sem1)
            checklist8 = currchecklist.objects.filter(owner_id=id).filter(curriculumCode__in=year4).filter(curriculumCode__in=sem2)
            subject_count = checklist.count()
            gwa = checklist.aggregate(Avg('subjectGrades'))
        except currchecklist.DoesNotExist:
            checklist = None
        context = {'id': id, 'info': info, 'checklist': checklist, 'subject_count': subject_count, 'gwa': gwa, 'checklist2': checklist2,'checklist3': checklist3,'checklist4': checklist4,'checklist5': checklist5,'checklist6': checklist6,'checklist7': checklist7,'checklist8': checklist8}
...
def sGradeSubmission1(request):
    if request.user.is_authenticated and request.user.is_student:
        OrderFormSet = inlineformset_factory(StudentInfo, currchecklist, fields=('owner','curriculumCode', 'subjectGrades'),widgets={'curriculumCode': forms.Select(attrs={"class": "form-control", "id":"instructorField"}), 'subjectGrades': forms.Select(attrs={"class": "form-control", "id":"instructorField"})}, extra =13, can_delete=False)
        id= request.user.id
        info = StudentInfo.objects.get(studentUser=id)
...
def sGradeSubmission2(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        if (request.method == 'POST'):
            ...
            try:
                student = crsGrade.objects.get(studentID_id=id)
...
def Student_Clearance(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        obj = hdClearanceForm.objects.last()
        ...
        if obj.studentID_id == info.studentUser_id:
            ...
            return render(request,'student/sOthers/sHd1.html')
    else:
            return redirect('index')
...
def Student_Transfer(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        info = StudentInfo.objects.get(studentUser=id)
        #context = {'id': id, 'info':info}
        obj = hdTransferCert.objects.last()
        #obj = Student.objects.get(id=studentid)
        context = {
            'id': id, 
            'info':info,
        ...
        if obj.studentID_id == info.studentUser_id:
            ...
            messages.error(request,'No Copy Yet')
            return render(request,'student/sOthers/sHd3.html')
    else:
            return redirect('index')
...
def Loa_Clearance(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        info = StudentInfo.objects.get(studentUser=id)
        #context = {'id': id, 'info':info}
        obj = loaClearanceForm.objects.last()
        #obj = Student.objects.get(id=studentid)
        context = {
            'id': id, 
            'info':info,
        ...
        if obj.studentID_id == info.studentUser_id:
            ...
            messages.error(request,'No Copy Yet')
            return render(request,'student/sOthers/sLoa1.html')
    else:
        return redirect('index')
...
def Loa_Form(request):
    if request.user.is_authenticated and request.user.is_student:
        ...
        if obj.studentID_id == info.studentUser_id:
            ...
            messages.error(request,'No Copy Yet')
            return render(request,'student/sOthers/sLoa2.html')
    else:
        return redirect('index')
...
def pta_request(request):
    if request.user.is_authenticated and request.user.is_chairperson:
        id= request.user.id
        info = FacultyInfo.objects.get(facultyUser=id)
        parttime = FacultyInfo.objects.filter(facultyWorkstatus='Part-Time')
...
#Study Plan Year of Admission BRRRRRRRRRRT
def studyplan1(request):
    if request.user.is_authenticated and request.user.is_student:
        OrderFormSet = inlineformset_factory(StudentInfo, studyPlan, fields=('studentinfo','admissionYr', 'curricula'),widgets={'admissionYr': forms.TextInput(attrs={"class": "form-control", "id":"instructorField"}), 'curricula': forms.Select(attrs={"class": "form-control", "id":"instructorField"})}, max_num=1, can_delete=False)
        id= request.user.id
        info = StudentInfo.objects.get(studentUser=id)
        formset = OrderFormSet(queryset=studyPlan.objects.none(), instance=info)
        if request.method =='POST':
            formset = OrderFormSet(request.POST, instance=info)
            if formset.is_valid():
                formset.save()
                ...
                formset = OrderFormSet(instance=info)
                if formset.is_valid():
                    formset.save()
        context = {'formset': formset }
    ...
    if request.user.is_authenticated and request.user.is_student:
        ...
        if (request.method == 'POST'):
            ...
            try:
                ...
                if applicant.remarks == "Returned":
                    if (request.method == 'POST'):
                        applicant.sdplan = request.FILES.get('studyPlanUp')
                        applicant.remarks = 'Submitted'
                        applicant.save()
                        return redirect('stdplncs') 
                else:
...
def faculty_view(request, faculty_id):
    if request.method == 'GET':
        ...
        facultyapp = FacultyApplicant.objects.get(pk=faculty_id)
        facultyapp_stat = request.POST.get('slct')
        facultyapp.remarks = facultyapp_stat
        facultyapp.save()
...
def cfacultyapplicant_sortedlist(request):
    ...
    return render(request, 'chairperson/cfaculty_applicant_sortedlist.html', context)
